...
Now, if you're the programmer of this game you could keep track of each player's health_score by have 5 different variables. (for players Zero to Four)

[source]
----
player_zero_health_score = 100
player_one_health_score = 100
player_two_health_score = 100
player_three_health_score = 100
player_four_health_score = 100
----
...
To get around this kind of problem we use a *list*. We could ask, "how many players are playing?", and then make the list that size. We know we need to track each player's health_score, so we create an list:

[source]
----
player_health_scores = [100, 100, 100, 100, 100]
----
...
player_health_scores[2] = 67  # player 2 just took a hit!

player_health_scores[1] = 105  # player one is getting stronger.

player_health_scores[3] = player_health_scores[3] - major_hit
...
Lists are *indexed* like that. Each list spot has an index number, starting at zero. See how we use the number 2 to _index_ into the player_health_score list?

Lists:

* Can store multiple values in a single variable
* Start counting from index position zero
* Elements can be builtin data types or/and Objects
...
list_of_letters = ['c','h','r','i','s']

mixed_data = ['one', 2, True]  # a string, a number and a boolean!
----
