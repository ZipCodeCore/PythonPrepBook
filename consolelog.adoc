...
We run `python3` (and let's get specific, v2 of this book relies on *at least* python 3.14 in this book.)
Don't use python version 2.
`python3` is what the pros use.
...
----
miles_per_hour = 55.0
print("Car's Speed: ", miles_per_hour)
----
...
Cool, huh? The ability to communicate with you is one of Python's most fundamental capabilities. And you've run two Python programs. Congratulations, you're a programmer. (Well, at least for today you are.)
...
print("We need", flour_amount, "cups of flour."); # -> We need 3.5 cups of flour.
----
...
regular_price * (1.0 - sale_percent_off)
----
...
*Boolean expressions* result in either a True or a False value. Example: "max_speed > 500.0" - this evaluates to either true or false depending on the value of max_speed.

=== Statements

A *statement* is just a line of Python.
All python statements have very specific patterns.
...
total_groceries = 38.99
sales_tax = total_groceries * sales_tax_rate

charge_to_card = total_groceries + sales_tax
----
...
This is referred to control flow. The first statement is executed, then the second, and so on, until the program reaches the end.

Some of the statements have expressions in them (like total_groceries * sales_tax_rate), while some are just simple *assignment* statements (like total_groceries = 38.99, where we assign the variable 'total_groceries' the value 38.99).
...
kilometers_cycled = number_of_hours_pedalled \
    * kilometers_per_hour \
    - (total_minutes_of_rest / 60)
----
...
It is _so important_ that *indentation* and *making sure you have everything aligned properly* is *one of the most important things to learn* about Python when you are a beginner.
Many beginners discover that the braces _{ }_ that other languages use to manage the way code is grouped together are missing from Python. Java, JavaScript, and many other languages use braces to handle how code gets grouped together.
...
if (mage_power > 120.0):
    max_magic = 500.0
    life_span = 800.0
    max_weapons = mage_power / max_power_per_weapon
    if (max_weapons < 150):
...
# some more code
print(mage_power, "is your Mage's Power rating.")
----

**Python does not use braces to group code together. Instead, it uses indentation.**
