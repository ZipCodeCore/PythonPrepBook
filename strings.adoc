...
price = 20
dollar_sign = "$"
price_tag = dollar_sign + str(price) # $20
...
work_report = "Today, " + name + " worked a total of " + hours_worked + " hours."
print(work_report)
----
...
Now let's use the 3 substring method-functions on first_name and extract and print out "Chris"

[source]
----
first_name = "Christopher"

print(first_name[0:5]) # "Chris"
----

We took the `first_name` string and extracted the characters 0 to 4. BUT, let's try to extract the string "stop" from the name.

[source]
----
first_name = "Christopher"
print(first_name[4:8]) # -> "stop"
----
...
first_name = "Christopher"
----
- Your turn to use the string slicing on first_name
...
Now, using the `first_name` variable again, let's reverse the string "STOP" to say "POTS".
...
----
first_name = "Christopher"
res = first_name[4: 8].upper() # -> "STOP"

rev = res[::-1] # -> POTS
print(rev) # -> POTS
----

Strings are perhaps the most important data type in almost any language. Being able to manipulate them easily and do powerful things with them in Python, makes you a better programmer.
