...
And many early programming problems you will come across deal with doing fairly easy math.
There are ways to do lots of useful things with numbers.
...
|// |Floor Division |Divide 2 numbers
...
=== Division(s) and Remainder

There are *two* types of division in Python: regular division and floor division. Plus, there is the remainder operator.

We know that we can do regular division.
If we have a simple program like this, we know what to expect:
...
Division always returns a floating-point number. It never returns an integer.

=== Floor Division

Floor division is the same as regular division, but it rounds down to the nearest integer. It always returns an integer. *Always*.

It is sometimes called Integer Division.
...
It's important to note that last one; `10 // 4` is 2.
Why?
Floor division is the same as regular division, but it rounds down to the nearest integer.

For example, `10 / 4` is 2.5, but `10 // 4` is 2.
...
print(square_root_two)
----

We will get "1.4142135623730951" twice in the output. That number (square_root_two)
...
[source]
----
square_root_two = math.pow(2, 0.5)
print(square_root_two)
----
...
You'll learn a lot more about working with numbers in your career as a programmer.
