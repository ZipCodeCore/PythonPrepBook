...
Wait! Notice how we have to ask range to *one beyond* (11) the last number we want added to the sum!
...
=== A Quick Word: Iterables

Notice that we've now written `for donut in list_of_donuts:` and `for j in range(5):` - two very different-looking things sitting after that `in`. Python has a name for anything that's allowed to sit there: an *iterable*. Something is iterable if a `for` loop knows how to walk through it, one item at a time.

Lists and ranges are iterables. So are strings - that's exactly what let us loop over "hello" letter by letter, back in the Strings chapter. As you meet more data structures later in this book - dictionaries, sets, tuples - you'll find every single one of them is iterable too, which means `for thing in that_data_structure:` will always just work.

You don't need to know *how* Python pulls this off yet - just get comfortable with the word "iterable," since we'll use it constantly from here on. Much later in the book, in a chapter called Iterators, we'll open the hood and see exactly what makes something iterable in the first place.
